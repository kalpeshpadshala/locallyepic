...
class Authorize_net
{
	...
	{
		$this->CI->curl->create($this->api_url);
			...
			if( $res !== FALSE )
			{
				if( isset($res->messages->resultCode) )
				{
					switch( $res->messages->resultCode )
					{
						...
						$this->error1 = isset($res->transactionResponse->errors->error->errorText) ? (string)$res->transactionResponse->errors->error->errorText : 'Unknown error';
	...
	{
		return isset($this->response->transactionResponse->transId) ? (int)$this->response->transactionResponse->transId : 0;
	}
	...
	public function getResponseCode() {
		...
		return isset($this->response->transactionResponse->responseCode) ? $this->response->transactionResponse->responseCode : NULL;
	}
	...
	{
		echo '<h1>Authorize.Net API Debug Info</h1>';
