...
class Wapi extends CI_Controller {
    ...
    private function loginLog($intcustomerid, $ipaddress, $device_token, $device_type, $email, $type){

        $q = "insert into tbl_logins set intcustomerid=?, ipaddress=?, device_token=?, device_type=?, email=?, tsloginLocal = DATE_SUB(now(),INTERVAL 5 HOUR), type=? ";
        $result = $this->db->query($q, array($intcustomerid, $ipaddress, $device_token, $device_type, $email, $type) );
...
public function record_deal_view(){
    ...
    public function signup_consumer(){
        ...
        if ($temp['stat'] == 1) {
            ...
            $this->loginLog($temp['last_id'], $_SERVER['REMOTE_ADDR'], $device_token, $device_type, $email, "Signup");
            ...
            $arr = array(
                'subject' => "Locally Epic : Verify The Account",
...
//            $this->_sendResponse(4); // password invalid
        ...
        $this->loginLog($user_info['user_id'], $_SERVER['REMOTE_ADDR'], $device_token, $device_type, $email, "Facebook");
    ...
    public function login() {
        ...
        $this->loginLog($user_info['user_id'], $_SERVER['REMOTE_ADDR'], $device_token, $device_type, $email, "Login");
                    ...
                    foreach ( $arrDeals as $deal) {
                       ...
                       log_message('error',$d);
                        ...
                        if ($user_info['device_type'] == 1) {
                            $this->sendPushNotificationIOS($arr, $user_info['device_token'], 'dgg.pem', '', 'product', "Locally Epic : Deals", $arr['badge'], 'default');
     ...
     public function push_send_user(){
       ...
       $arr = array(
                ...
                if ($user_info['device_type'] == 1) {
                    ...
                    $this->sendPushNotificationIOS($push_arr, $user_info['device_token'], 'push_pro.pem', '123456', 'product', "Locally Epic : Deals", $push_arr['badge'], 'default');
                } else if ($user_info['device_type'] == 2) {
                    $this->send_android_push($arr);
...
$qtest = "
    ...
    public function get_catname() {
        ...
        $q = "update tbl_customer set user_cat = concat_ws(',',user_cat, 102) where find_in_set(102,user_cat)=0 and user_id=?;";
        $result = $this->db->query($q, array( $user_id));
        $q = "update tbl_customer set user_cat = concat_ws(',',user_cat,113) where find_in_set(113,user_cat)=0 and user_id=?";
        $result = $this->db->query($q, array( $user_id));
        $q = "update tbl_customer set user_cat = concat_ws(',',user_cat,123) where find_in_set(123,user_cat)=0 and user_id=?";
        $result = $this->db->query($q, array( $user_id));
            ...
            SELECT
                ...
                (
                    ...
                    AND (((acos(sin(($lat*pi()/180)) * sin((`latitude`*pi()/180))+cos(($lat*pi()/180)) * cos((`latitude`*pi()/180))
                             * cos((($lon - `longitude`)*pi()/180))))*180/pi())*60*1.1515) <=15 and deal_end_time > UNIX_TIMESTAMP() and deal_time < UNIX_TIMESTAMP() order by id desc limit 1
            ) as livedeals
            FROM
    ...
    public function forgot_password(){
        ...
        $ua = $this->get_useragent();
        $link = "http://db.locallyepic.com/" . "c_support/forgot_password?secret_token=" . $secret_token . "&ua=" . $ua."&is_consumer=1";
        $email_message = $this->load->view('email/forgot_password', $data = array('link' => $link, 'dt' => $dt, 'name' => $info['name']), TRUE);


        $arr = array(
            'subject' => "Locally Epic : Forgot Password",
            'message_body' => $email_message,
            'to' => $email,
        );
        $this->send_email($arr);
    ...
    public function send_reset_password() {
        ...
        $arr = array(
            'subject' => "Locally Epic : Reset Password",
            'message_body' => $email_message,
            'to' => $email,
        );
        $this->send_email($arr);
...
echo "start push.<br>";
    ...
    $this->sendPushNotificationIOS($arr, $user_info['device_token'], 'push_pro.pem', '123456', 'product', "Locally Epic : Deals", 1, 'default');
...
function sendOneSignalMessage($push_arr, $deviceToken, $alert){
    log_message('error','2757 right inside of sendOneSignalMessage');
    ...
    $fields = array(
      ...
      'data' => array("user_id" => $push_arr['user_id'], "deal_id"=> $push_arr['deal_ids']),
      'contents' => $content
    );
...
function sendPushNotificationIOS($push_arr, $deviceToken, $certFile, $certPass, $push_method, $alert, $badge, $sound) 
     { 
        ...
        $this->sendOneSignalMessage($push_arr, $deviceToken, $alert);
