...
class Wapi4 extends CI_Controller {
    ...
    private function apilogupdate($id, $data){        
        $q = "update apicalls set apiresponse=? where id=?";
        $result = $this->db->query($q, array($data,$id ));
    }
...
public function record_deal_view(){
    ...
    public function signup_consumer(){
        ...
        $_POST= json_decode($response, TRUE);
        $this->postvars= json_decode($response, TRUE);
        ...
        $age = $this->is_require($this->postvars, 'age');
        $gender = $this->is_require($this->postvars, 'gender');

        $p = $_POST;
        $p["password"]="";
        $api_id = $this->apilog(__FUNCTION__, 0, $p, '');
        // $this->apilogupdate($api_id,$data);
        ####################################### 
        ...
        //echo "<pre>";print_r($t);exit;
        if ($t->cnt >= 1) {
            $this->apilogupdate($api_id,"Email Already Exists");
            $this->_sendResponse(2); //email already exists
        }
    ...
    public function get_active_deals(){
        ...
        $q = "
            ...
            select 
                    ...
                    (select count(*) from loyaltyprograms where loyaltyprograms.shop_id=tbl_shop.shop_id ) as blnHasCL,
                    (select count(*) from loyaltyprograms where loyaltyprograms.shop_id=tbl_shop.shop_id ) as blnHasCL,
                    round(((acos(sin(($latitude*pi()/180)) * sin((`latitude`*pi()/180))+cos(($latitude*pi()/180)) * cos((`latitude`*pi()/180))
                 * cos((($longitude - `longitude`)*pi()/180))))*180/pi())*60*1.1515) AS `distance`,
                    CONCAT(
                        TIMESTAMPDIFF(day,now(),tbl_deal.deal_end) , 'd ',
                        MOD( TIMESTAMPDIFF(hour,now(),tbl_deal.deal_end), 24), 'h ',
                        MOD( TIMESTAMPDIFF(minute,now(),tbl_deal.deal_end), 60), 'm '
                        ) as offertime
        ...
        if (isset($_POST["returnFavoritesOnly"]) && $_POST["returnFavoritesOnly"]=='1' ){

            $q = $q." and tbl_deal.id in (select deal_id from tbl_deal_favorite where user_id = '$user_id')";
        }
    ...
    public function get_push_deals(){
        ...
        $q = "
            ...
            select 
                    ...
                    tbl_deal.website,
                    0 as share_count,
                    tbl_category.cname as category,
                    tbl_deal.status,
                    tbl_shop.shop_name,
                    tbl_shop.pin,
                    tbl_shop.shop_cats,
            ...
            from push_notes 
                 join tbl_deal on push_notes.deal_id = tbl_deal.id 
                 join tbl_shop on tbl_deal.shop_id = tbl_shop.shop_id
                 join tbl_category on tbl_shop.shop_cats = tbl_category.cid
...
$qtest = "
            ...
            order by push_notes.deal_id desc
    ...
    public function searchdeals(){
        ...
        $this->apilog(__FUNCTION__, -1, $_POST, '');
...
//                $info[$k]['is_active'] = '1';
    ...
    private function _sendResponse($status_code = 200) {
        ...
        $this->result['status_code'] = $status_code;
         ...
         $q = "insert into sendresponses set json=?";
        $result = $this->db->query($q, array(json_encode($this->result) ));
